...
def soft_sink_horn_to_hard(log_alpha_w_noise):

    to_change = log_alpha_w_noise.clone()

    hard_perms_inf = my_matperm2listperm(to_change)
    perm_mat_mask = my_listperm2matperm(hard_perms_inf)

    log_alpha_w_noise = log_alpha_w_noise*perm_mat_mask
    f_relu = NSReLU()
    log_alpha_w_noise = f_relu(log_alpha_w_noise)
    # perm_mat[perm_mat > 0] = 1
